...
def test_readme_local_recipe_matches_action_output():
    ...
    for path in ("check_svg.py", "style/default.json"):
        url = f"https://raw.githubusercontent.com/meawoppl/visual-pr/v4/{path}"
        assert url in README, f"README missing {url}"
    assert 'base="https://raw.githubusercontent.com/${ACTION_REPO:-meawoppl/visual-pr}/${ACTION_REF:-v4}"' in ACTION
...
def test_versioning_is_consistent():
    assert "@v1" not in README and "@v2" not in README and "@v3" not in README, "README must not point consumers at a frozen major"
    assert "meawoppl/visual-pr@v4" in README
    assert "${ACTION_REF:-v4}" in ACTION and "${ACTION_REF:-v2}" not in ACTION
    changelog = (ROOT / "CHANGELOG.md").read_text()
    assert changelog.startswith("# Changelog") and "## v4.0.0" in changelog and "## v3.0.0" in changelog and "## v2.0.0" in changelog and "## v1" in changelog
...
def test_sweep_action_inputs_are_documented():
    ...
    assert "meawoppl/visual-pr/sweep@v4" in section
